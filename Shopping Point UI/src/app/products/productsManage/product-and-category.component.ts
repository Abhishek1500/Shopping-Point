import { Component, OnInit, ViewChild } from '@angular/core';
import { ProductsService } from '../../Services/products.service';
import { Product } from '../../Models/Product';
import { ToastrService } from 'ngx-toastr';
import { CategorySummary } from '../../Models/category-summary';
import { NgForm } from '@angular/forms';

@Component({
  selector: 'app-product-and-category',
  templateUrl: './product-and-category.component.html',
  styleUrls: ['./product-and-category.component.css']
})
export class ProductAndCategoryComponent implements OnInit {

  mindate="";

  shouldshow=true;
  componentType="0";
  private allProducts:Product[]=[];
  public allCategory:CategorySummary[]=[];
  public products:(string|number|Date)[][]=[];
  public categories:string[]=[];
  public isAdmin=false;
  public isEditing=false;
  public productId=0;
  public loading=false;
  public temp:any;
  public imgfiedmainatiner="";
  

  newProduct={
    id:0,
    productName:"",
    productComapny:"",
    dateOfManuFacture:"",
    quantity:"",
    description:"",
    imageurl:"",
    price:"",
    categoryId:"",
    askedQuantity:0,
  }

  newCategory={
    id:0,
    categoryName:""
  }


  constructor(private productService:ProductsService,
    private toastr:ToastrService) { }

  ngOnInit(): void {
    this.mindate=new Date().toISOString().split("T")[0];
    this.loading=true;
    this.componentType="0";
    this.productService.getAllProducts().subscribe({
      next: data=>{
        this.allProducts=data;
        this.products=this.convertToTableFormat(this.allProducts)
        this.loading=false;
      },
      error:er=>{
        this.toastr.error(er.error.message)
        this.loading=false;
        return;
      }
    }
    )

    this.loading=true;

    this.productService.getAllCAtegories().subscribe({
      next:data=>{
        this.allCategory=data;
        this.categories=this.convertCategoryToTableFormat(this.allCategory);
        this.loading=false
      },
      error:err=>{
        this.toastr.error(err.error.message)
        this.loading=false;
        return;
      }
    })

    this.isAdmin=true;
  }


  parentfunction(data:any){
    if(this.componentType=="1"){
      return;
    }
    this.productId=data;
    this.makeEditproductArea();
  }



  openModal(){
    document.getElementById("openModalButton")?.click();
  }
  openModal2(){
    document.getElementById("openModalButton2")?.click();
  }

  addProduct(){
    this.loading=true
    this.productService.addNewProduct({
      productName: this.newProduct.productName,
      productCompany: this.newProduct.productComapny,
      dateOfManuFacture: this.newProduct.dateOfManuFacture,
      quantity: this.newProduct.quantity,
      description: this.newProduct.description,
      imageurl: this.newProduct.imageurl,
      price: this.newProduct.price,
      categoryId: this.newProduct.categoryId})
      .subscribe({
        next:data=>{
          this.allProducts.push(data);
          this.products=this.convertToTableFormat(this.allProducts)
          this.toastr.success("The product is Added SuccessFully");
          this.loading=false
        },
        error:err=>{
          this.toastr.error(err.error.message);
          this.loading=false
      }
    })
  }

  deleteProduct(){
    this.loading=true;
    this.productService.deleteProduct(this.newProduct.id).subscribe({
      next:data=>{
        let index=this.allProducts.findIndex(x=>x.id==this.newProduct.id)
        this.allProducts.splice(index,1);
        this.products=this.convertToTableFormat(this.allProducts);
        this.toastr.success("Product deleted SuccessFully")
        this.loading=false;
      },
      error:err=>{
        this.toastr.error(err.error.message);
        this.loading=false;
      }
    })
  }

  editProduct(){
    
    this.loading=true;
    this.productService.updateProduct(this.newProduct.id,{
        productName: this.newProduct.productName,
        productCompany: this.newProduct.productComapny,
        dateOfManuFacture: this.newProduct.dateOfManuFacture,
        quantity: this.newProduct.quantity,
        description: this.newProduct.description,
        imageurl: this.newProduct.imageurl,
        price: this.newProduct.price,
        categoryId: this.newProduct.categoryId})
  .subscribe({
      next:data=>{
        let index=this.allProducts.findIndex(x=>x.id==this.newProduct.id)
        this.allProducts[index]=data;
        this.products=this.convertToTableFormat(this.allProducts)
        this.toastr.success("The Product is Updated SuccessFully");
        this.loading=false;
      },
      error:err=>{
        this.toastr.error(err.error.message);
        this.loading=false;
      }
    })
  }

  addCategory(){
    this.loading=true;
    this.productService.addNewCategory({categoryName:this.newCategory.categoryName}).subscribe({
      next:data=>{
        this.loading=false   
        this.allCategory.push({
          id:data.id,
          categoryName:data.categoryName,
          products:[]
        });
        this.categories=this.convertCategoryToTableFormat(this.allCategory)
        this.toastr.success("The category is Added SuccessFully");
      },
      error:err=>{
        this.toastr.error(err.error.message);
        this.loading=false   
      }
    })
  }

  editCategory(){
    this.loading=true;
    this.productService.updateCategory(this.newCategory.id,this.newCategory.categoryName).subscribe({
      next:data=>{
        let index=this.allCategory.findIndex(x=>x.id==this.newCategory.id);
        this.allProducts.forEach(x=>{x.categoryName=this.newCategory.categoryName})
        this.products=this.convertToTableFormat(this.allProducts.filter(x=>x.quantity==0));
        this.allCategory[index].categoryName=this.newCategory.categoryName.toUpperCase().trim();
        this.categories=this.convertCategoryToTableFormat(this.allCategory)
        this.toastr.success("The category is Updated SuccessFully");
        this.loading=false;
      },
      error:err=>{
        this.toastr.error(err.error.message);
        this.loading=false;
      }
    })
  }


  deleteCategory(){
    this.loading=true;
    let value=(<HTMLInputElement>document.getElementById("temp"))?.value;
    if(value==""){
      this.toastr.error("please selete the category First in above table")
      this.loading=false;
      return;
    }
    let cat=this.allCategory.find(x=>x.categoryName===value);
    if(cat!=null)
    this.productService.deleteCategory(cat.id).subscribe({
      next:data=>{
        let index=this.allCategory.findIndex(x=>x.id===cat?.id);
        this.allProducts=this.allProducts.filter(x=>x.categoryName!=this.allCategory[index].categoryName);
        this.products=this.convertToTableFormat(this.allProducts.filter(x=>x.quantity==0))
        this.allCategory.splice(index,1);
        this.categories=this.convertCategoryToTableFormat(this.allCategory)
        this.toastr.success("The category is Deleted SuccessFully");
        this.loading=false;
      },
      error:err=>{
        this.toastr.error(err.error.message);
        this.loading=false;
      }
    })
  }

  makeEditproductArea(){
    this.isEditing=true;
    this.imgfiedmainatiner=""
    let p=this.allProducts.find(x=>x.id===this.productId);
    if(p==null){
      return;
    }
    this.newProduct.id=Number(p.id);
    this.newProduct.productName=p.productName,
    this.newProduct.productComapny=p.productCompany;
    this.newProduct.price=p.price+"";
    this.newProduct.quantity=p.quantity+"";
    this.newProduct.description=p.description;
    this.newProduct.imageurl=p.imageurl;
    this.newProduct.dateOfManuFacture=(p.dateOfManuFacture+"").split("T")[0];
    let cat=this.allCategory.find(x=>x.categoryName==p?.categoryName);
    if(cat!=null){
    this.newProduct.categoryId=cat?.id+"";
    }
    this.openModal()
  }

  makeAddproductArea(form:any){
    this.isEditing=false;
    this.imgfiedmainatiner="";
    this.newProduct.imageurl="data:image/jpeg;base64,"
    form.control.reset()
    this.openModal()
  }


  makeEditCategoryReady(){
    let value=(<HTMLInputElement>document.getElementById("temp"))?.value;
    if(value==""){
      this.toastr.error("please selete the category First in above table")
      return;
    }
    this.isEditing=true;
    let cat=this.allCategory.find(x=>x.categoryName===value);
    if(cat!=null){
      this.newCategory.id=cat.id
      this.newCategory.categoryName=cat.categoryName
    }
    this.openModal2();
  }

  makingAddCategoryArea(form:any){
    this.isEditing=false;
    form.control.reset()
    this.openModal2()
  }


  changeComponentFunctionality(){
    if(this.componentType==="1"){
      this.shouldshow=false;
      this.products=this.convertToTableFormat(this.allProducts.filter(x=>x.quantity===0));
      if(this.products.length==0){
        this.toastr.error("No Product is Out Of Stock");
      }
    }else{
      this.products=this.convertToTableFormat(this.allProducts);
      this.shouldshow=true;
    }
  }

  onFileSelected(event: any): void {
    const file: File = event.target.files[0];
    if (file&&(file.type==='image/jpeg'||file.type==='image/png')) {
      const reader = new FileReader();
    reader.onload = (e:any) => {
      const base64Data = reader.result as string ;
      if(this.newProduct.imageurl!=base64Data)
        this.newProduct.imageurl=base64Data;
      else
      this.toastr.error("image is same as present image");
    };
    reader.readAsDataURL(file);
    }else{
      this.toastr.error("please file should be jpeg or png format")
    }
  }


  private processProducts(el:Product){
    return [el.id,el.imageurl,el.productName,el.productCompany,el.price,el.quantity===0?"OutOfStock":el.quantity,el.categoryName]
  }

  private convertToTableFormat(arr:Product[]){
    let temp:(string|number|Date)[][]=[];
    arr.forEach(el => {
      temp.push(this.processProducts(el));  
    });
    return temp;
  }

  private convertCategoryToTableFormat(arr:CategorySummary[]){
    let cat:string[]=[];
    arr.forEach(e=>{
      cat.push(e.categoryName)
    });
    return cat;
  }

}
